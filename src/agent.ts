...
type Agent =
  | InstanceType<typeof SocksProxyAgent>
  | InstanceType<typeof HttpProxyAgent>
...
// @ts-ignore
const setAxiosAgent = (axiosRequestConfig: any, agentSocket: AgentSocket) => {
  return {
    ...(axiosRequestConfig || {}),
    httpAgent: getAgent(agentSocket, 'http'),
    httpsAgent: getAgent(agentSocket, 'https'),
  };
};
